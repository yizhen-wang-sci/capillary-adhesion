...
from a_package.simulation import (
    load_config, save_config, unroll_sweep, reset_logging, switch_log_file,
    CaseDir, RunDir, SimulationIO, get_timestamp,  
)
...
def main():
    ...
    with case_dir.bookkeep() as entry:
        ...
        for config in unroll_sweep(config_origin):
            run_name = "--".join([
                get_timestamp(),
                f"pressure{config['simulation']['constraint']['pressure']}",
            ...
            io = _run_constant_pressure(config, run_dir.data_dir)
            ios.append(io)

    # post-process
    for run_name, io in zip(runs, ios):
        run_dir = RunDir(case_dir / run_name)
        _visualize_constant_pressure(io, run_dir.visuals_dir)
...
def _run_constant_pressure(config, output_path):
...
def _visualize_constant_pressure(io: SimulationIO, output_dir):
