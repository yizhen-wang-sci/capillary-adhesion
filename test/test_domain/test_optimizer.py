...
def test_problem(decompose_stitch, comm_world):
    ...
    result = optimizer.solve_minimisation(problem, x0=sinusoidal_field[*decomposition.icoords])
    solved_field = result['x'].reshape(decomposition.nb_subdomain_grid_pts)
    print(result)
    assert result['success']
    assert result['nit'] < optimizer.max_inner_iter
    np.testing.assert_allclose(solved_field, mean_field[*decomposition.icoords])
