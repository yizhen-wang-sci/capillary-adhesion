...
def test_unconstrained_numopt():
    ...
    result = optimizer.solve_minimisation(num_opt, x0=np.array([0., 1.]))
...
def test_eq_constrained_numopt():
    ...
    result = optimizer.solve_minimisation(num_opt, x0=np.array([0., 1.]), lam0=0., alpha0=1e1)
...
def test_bound_constrained_numopt():
    ...
    num_opt = types.SimpleNamespace(get_x=get_x, set_x=set_x, get_f=get_f, get_f_Dx=get_f_Dx, x_lb=1., x_ub=2.)

    optimizer = Optimizer(max_loop=1)
    result = optimizer.solve_minimisation(num_opt, x0=np.array([0., 1.]))

    assert np.all(np.isclose(result['primal'], 1.))
...
def test_eq_and_bound_constrained_numopt():
    ...
    num_opt = types.SimpleNamespace(get_x=get_x, set_x=set_x, get_f=get_f, get_f_Dx=get_f_Dx, get_g=get_g, get_g_Dx=get_g_Dx, x_lb=np.array([2., 0.]), x_ub=10.)

    optimizer = Optimizer(max_loop=50)
    result = optimizer.solve_minimisation(num_opt, x0=np.array([0., 0.]), lam0=0., alpha0=1e1, beta0=1e-3)
